...
def transfer_puppy(puppy, new_shelter, old_shelter=None):
    ...
    db.session.add_all([puppy, new_shelter])
    if old_shelter:
...
def check_in_puppy(puppy, shelter=None):
    ...
    Args:
      puppy: Puppy - puppy to check into the shelter
      shelter: Shelter - the shelter where we intend to house the Puppy
    ...
    open_shelters = Shelter.query.filter(
...
def process_puppy_adoption(puppy_id, adopter_ids):
    ...
    Args:
      puppy_id: int - the id of the puppy to be adopted
      adopter_ids: list(int) - a list of ids belonging to adopters

    Returns:
      A status message indicating who adopted the puppy.
    """
    puppy = Puppy.query.filter_by(id=puppy_id).one()
    adopters = Adopter.query.filter(Adopter.id.in_(adopter_ids)).all()
